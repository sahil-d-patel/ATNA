"""Load pipeline YAML: snapshot month, raw paths, processed output dir."""
...
# Repository root (src/etl/config.py -> parents[2])
...
class AtnaConfig:
    """Resolved pipeline paths for one MVP snapshot."""
...
def _format_templates(data: Mapping[str, Any], snapshot_id: str) -> tuple[str, str, str, str]:
    raw = data["raw"]
    out = data["output"]
    ...
    return on_time, t100, master, processed


def load_config(path: Path | str | None = None) -> AtnaConfig:
    """Load `config/atna.yaml`, validate snapshot_id, resolve paths under repo root.

    Relative paths in YAML are resolved against ``REPO_ROOT`` (this package tree),
    same pattern as ``scripts/download/download_bts_data.py``.
...
def validate_paths(cfg: AtnaConfig) -> None:
    """Ensure raw CSVs exist (development / pre-ETL check)."""
    missing: list[str] = []
    for label, p in (
        ("raw.on_time_glob (resolved)", cfg.raw_on_time),
        ("raw.t100_glob (resolved)", cfg.raw_t100),
        ("raw.master_airport_file", cfg.raw_master_airport),
    ):
        if not p.is_file():
            missing.append(f"  {label}: {p}")
    if missing:
        raise FileNotFoundError("Expected raw files not found:\n" + "\n".join(missing))
